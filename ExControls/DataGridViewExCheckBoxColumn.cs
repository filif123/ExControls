    ...
    public class DataGridViewExCheckBoxCell : DataGridViewCheckBoxCell
    {
            ...
            DataGridViewPaintParts paintParts)
        {
            ...
            if ((paintParts & DataGridViewPaintParts.Background) == DataGridViewPaintParts.Background)
            {
                ...
                using SolidBrush cellBackground = selected ? new SolidBrush(cellStyle.SelectionBackColor) : new SolidBrush(cellStyle.BackColor);
            ...
            Point pt = GetCheckBoxPoint(cellBounds);
            var rect = new Rectangle(pt, new Size(16, 16));
            using var background = new SolidBrush(SquareBackColor);
            g.FillRectangle(background, rect);

            Color colorMark = _hover ? HighlightColor : state != DataGridViewElementStates.ReadOnly ? MarkColor : Color.DimGray;
            Color colorBorder = _hover ? HighlightColor : state != DataGridViewElementStates.ReadOnly ? BorderColor : Color.DimGray;

            var bs = ButtonState.Normal;

            if (formattedValue is bool val)
            {
                bs = val == false ? ButtonState.Normal : ButtonState.Checked;
            }
        ...
        {
            int x = cellBounds.Width / 2 - 8;
            int y = cellBounds.Height / 2 - 8;
        ...
        protected override void OnMouseMove(DataGridViewCellMouseEventArgs e)
        {
            ...
            if (DataGridView == null)
                return;
            bool mouseInContentBounds = _hover;
            _hover = GetContentBounds(e.RowIndex).Contains(e.X, e.Y);
            if (mouseInContentBounds != _hover)
            {
