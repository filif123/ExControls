...
{
    /// <summary>
    /// This class contains static methods to measure and draw buttons and text into the control graphics. This class cannot be inherited.
    /// </summary>
    public static class ExButtonsRenderer
    {
        /// <summary>
        ...
        public static void DrawDropDownButton(Graphics g, Rectangle rec, Color backColor, Color borderColor, Color arrowColor)
        {
            ...
            int arrowX = rec.X + rec.Width / 2 - 3;
            int arrowY = rec.Y + rec.Height / 2 - 1;
        ...
        public static BoxAndTextRectangle GetBoxAndTextRectangle(IDeviceContext g, ICheckableExControl c, int boxSize, int boxOffset)
        {
            ...
            int halfw = (int)Math.Round((c.ClientRectangle.Width / 2d) - (boxSize / 2d));
            int halfh = (int)Math.Round((c.ClientRectangle.Height / 2d) - (boxSize / 2d));

            Size texts = TextRenderer.MeasureText(g, c.Text, c.Font);
            int halftextw = texts.Width / 2;
            ...
            switch (c.TextAlign)
            {
                ...
                case ContentAlignment.MiddleLeft:
                    boxtext.TextRectangle = new Rectangle(0, halfh, texts.Width, texts.Height);
                    if (c.RightToLeft == RightToLeft.Yes)
                        boxtext.TextRectangle = new Rectangle(c.ClientRectangle.Width - texts.Width, c.ClientRectangle.Height / 2 - texts.Height / 2, texts.Width, texts.Height);
                    boxtext.TextRectangle = OffsetText(boxtext.TextRectangle, c, boxSize, boxOffset);
                    break;

                case ContentAlignment.MiddleCenter:
                    boxtext.TextRectangle = new Rectangle(c.ClientRectangle.Width / 2 - texts.Width / 2, c.ClientRectangle.Height / 2 - texts.Height / 2, texts.Width, texts.Height);
                    break;
                ...
                case ContentAlignment.BottomLeft:
                    ...
                    if (c.RightToLeft == RightToLeft.Yes)
                        boxtext.TextRectangle = new Rectangle(c.ClientRectangle.Width - texts.Width, c.ClientRectangle.Height - texts.Height, texts.Width, texts.Height);
                    boxtext.TextRectangle = OffsetText(boxtext.TextRectangle, c, boxSize, boxOffset);
                    break;

                case ContentAlignment.BottomCenter:
                    boxtext.TextRectangle = new Rectangle(c.ClientRectangle.Width / 2 - halftextw, c.ClientRectangle.Height - texts.Height, texts.Width, texts.Height);
        ...
        private static Rectangle OffsetText(Rectangle rec, ICheckableExControl c,int boxSize, int boxOffset, bool left = true)
        {
            if (left)
            {
                if (c.CheckAlign == ContentAlignment.TopLeft || c.CheckAlign == ContentAlignment.MiddleLeft || c.CheckAlign == ContentAlignment.BottomLeft)
                {
            ...
            {
                if (c.CheckAlign == ContentAlignment.TopRight || c.CheckAlign == ContentAlignment.MiddleRight || c.CheckAlign == ContentAlignment.BottomRight)
                {
                    rec = c.RightToLeft == RightToLeft.No
                        ? ChangeRecValue(rec, -boxSize - boxOffset, 0)
                        : ChangeRecValue(rec, boxSize + boxOffset, 0);
